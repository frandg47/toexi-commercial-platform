...
export function SalesList() {
  ...
  const [warrantySettlementAccountId, setWarrantySettlementAccountId] =
    useState("");
  const [warrantySettlementMethodId, setWarrantySettlementMethodId] =
    useState("");
  const [warrantySettlementInstallments, setWarrantySettlementInstallments] =
    useState("");
  ...
  const settlementInstallmentOptions = useMemo(
    () =>
      paymentInstallments.filter(
        (inst) => inst.payment_method_id === Number(warrantySettlementMethodId),
      ),
  ...
  const warrantyPriceDiff = useMemo(() => {
    ...
    const differenceUsd = Number(
      (replacementTotalUsd - originalTotalUsd).toFixed(2),
    );
  ...
  const selectedSettlementAccount = useMemo(
    () =>
      displayAccounts.find(
        (account) => String(account.id) === String(warrantySettlementAccountId),
      ) || null,
  ...
  const settlementMultiplier = useMemo(() => {
    ...
    const selectedInstallment = settlementInstallmentOptions.find(
      (inst) => inst.installments === Number(warrantySettlementInstallments),
    );
    return Number(
  ...
  const warrantySettlementPreview = useMemo(() => {
    ...
    if (currency === "ARS") {
      ...
      const amount = Number(
        (absoluteUsd * Number(fxRate) * multiplier).toFixed(2),
      );
      return {
        currency,
        amount,
  ...
  const load = useCallback(async () => {
    try {
      ...
      const auditIds = Array.from(
        new Set(
          rows.flatMap((s) => [s?.voided_by, s?.updated_by]).filter(Boolean),
        ),
      );
  ...
  const openEditSale = (sale) => {
    ...
    setEditChannelId(
      sale?.sales_channel_id ? String(sale.sales_channel_id) : "",
    );
    setEditOpen(true);
  };
  ...
  const handleSaveEdit = async () => {
    ...
    if (
      editingSale.status === "anulado" ||
      editingSale.status === "cancelado"
    ) {
  ...
  const openWarrantyDialog = async (sale) => {
    ...
    if (sale.status === "anulado" || sale.status === "cancelado") {
      toast.error(
        "No se puede gestionar garantia sobre una venta anulada o cancelada",
      );
      return;
    }
    ...
    try {
      const [
        ...
        supabase
          .from("sale_items")
          .select(
            "id, sale_id, variant_id, product_name, variant_name, color, quantity, imei, usd_price, subtotal_usd",
          )
        ...
        supabase
          .from("product_variants")
          .select(
            "id, variant_name, color, stock, usd_price, products(name, active, inventory_tracking_mode)",
          )
          .gt("stock", 0)
          .order("id", { ascending: true }),
      ]);
      ...
      setWarrantyReplacementRows([
        createWarrantyReplacementRow(
          defaultItem.variant_id || validVariants[0]?.id || "",
          1,
        ),
      ]);
      setWarrantyProductSearch("");
  ...
  const handleProcessWarranty = async () => {
    ...
    for (const row of validReplacementRows) {
      ...
      if (isSerialTrackedVariant(replacementVariant)) {
        ...
        if (!inventoryUnit) {
          toast.error(
            "No se encontró una unidad disponible para el reemplazo indicado",
          );
          return;
        }
    ...
    <div className="pb-6 space-y-4">
              ...
              >
                ...
                <div className="flex items-center gap-3 mt-2 flex-wrap">
                  ...
                  {s.sales_channel_name && (
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline">{s.sales_channel_name}</Badge>
                    </div>
                  )}
                </div>
                        ...
                        return (
                          <div className="flex justify-between border-b py-1 text-green-600">
                            <span>
                              Crédito canje (
                              {items.length === 1
                                ? `${items[0].product_name} ${items[0].variant_name}`
                          ...
                          <div className="pt-1">
                            ...
                            <ul className="mt-1 space-y-1">
                              {Object.entries(updatedFields).map(
                                ([fieldKey, payload]) => (
                                  <li key={fieldKey}>
                                    {renderUpdatedField(fieldKey, payload)}
                                  </li>
                                ),
                              )}
                            </ul>
                  ...
                  <div className="text-xs border-l-2 border-amber-500 rounded p-3 mt-3 bg-amber-50 dark:bg-amber-950/20">
                    ...
                    <div className="space-y-3 text-amber-700 dark:text-amber-300">
                      {saleWarranties.map((warranty) => (
                        <div
                          key={warranty.id}
                          className="rounded-md border border-amber-200/60 dark:border-amber-900/60 p-3"
                        >
                          <p>
                            <strong>Fecha:</strong>{" "}
                            {warranty.created_at
                              ? new Date(warranty.created_at).toLocaleString(
                                  "es-AR",
                                  {
                                    timeZone: AR_TIMEZONE,
                                    hour12: false,
                          ...
                          <p>
                            ...
                            {formatWarrantyBucket(
                              warranty.returned_stock_bucket,
                            )}
                          </p>
                          {Math.abs(
                            Number(warranty.price_difference_usd || 0),
                          ) > 0.009 && (
                            <p>
                              <strong>
                                {warranty.settlement_type === "store_credit"
                                  ? "Saldo a favor:"
                                  : warranty.settlement_type ===
                                      "customer_payment"
                                    ? "Diferencia cobrada:"
                                    : "Diferencia:"}
                              </strong>{" "}
                              ...
                              {Number(
                                warranty.settlement_amount ||
                                  warranty.store_credit_usd ||
                                  Math.abs(warranty.price_difference_usd || 0),
                              ...
                              (
                              {Number(
                                warranty.price_difference_usd || 0,
                              ).toFixed(2)}{" "}
                              USD)
                ...
                <div className="text-right mt-3 space-y-1">
                  <div className="text-sm text-muted-foreground">
                    Subtotal: $
                    {(
                      Number(s.total_ars) + Number(s.discount_amount || 0)
                    ).toLocaleString("es-AR")}
                  </div>
              ...
              >
                ...
                <SelectContent>
                  {editingSale?.seller_id &&
                    !sellerOptions.some(
                      (seller) => seller.id_auth === editingSale.seller_id,
                    ) && (
                  ...
                  {sellerOptions.length === 0 ? (
                    ...
                    sellerOptions.map((seller) => (
                      <SelectItem key={seller.id_auth} value={seller.id_auth}>
              ...
              >
                ...
                <SelectContent>
                  {editingSale?.sales_channel_id &&
                    !channels.some(
                      (ch) => ch.id === editingSale.sales_channel_id,
                    ) && (
      ...
      >
        <DialogContent className="w-[90vw] sm:max-w-xl md:max-w-2xl max-h-[85svh] overflow-y-auto rounded-2xl p-4 sm:p-6">
          <DialogHeader>
            <DialogTitle>
              Gestionar garantia{" "}
              {warrantySale ? `#${warrantySale.sale_id}` : ""}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => {
                  ...
                  setWarrantyReplacementRows([
                    createWarrantyReplacementRow(nextReplacement?.id || "", 1),
                  ]);
                }}
              >
                <SelectTrigger>
            ...
            <div className="rounded-md border bg-muted/30 p-3 text-sm space-y-1">
              ...
              <p>
                ...
                {Number(
                  warrantyPriceDiff.replacementTotalUsd || 0,
                ).toLocaleString("es-AR", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
                ...
                {fxRate &&
                  Math.abs(warrantyPriceDiff.differenceUsd) > 0.009 && (
                    <span className="text-xs">
                      ...
                      ($
                      {Math.abs(
                        warrantyPriceDiff.differenceArs || 0,
                      ).toLocaleString("es-AR", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}{" "}
                  ...
                  {warrantyPriceDiff.differenceUsd < -0.009 && (
                    ...
                    >
                      <input
                        ...
                        onChange={() =>
                          setWarrantySettlementMode("store_credit")
                        }
                        className="mt-0.5 h-4 w-4"
                      />
                      <div>
                  ...
                  {warrantyPriceDiff.differenceUsd > 0.009 && (
                    ...
                    >
                      <input
                        ...
                        onChange={() =>
                          setWarrantySettlementMode("customer_payment")
                        }
                        className="mt-0.5 h-4 w-4"
                      />
                      <div>
                ...
                <div className="rounded-md border border-dashed p-3 text-sm">
                  {warrantyPriceDiff.differenceUsd < -0.009 ? (
                    <p className="text-sky-700 dark:text-sky-300">
                      <strong>Cambio directo:</strong> El producto de reemplazo
                      tiene un precio de venta $
                      {Math.abs(warrantyPriceDiff.differenceUsd).toFixed(2)} USD
                      menor. El costo (COGS) de la venta original se actualizará
                      al costo del producto entregado.
                    </p>
                  ) : (
                    <p className="text-amber-700 dark:text-amber-300">
                      <strong>Cambio directo:</strong> El producto de reemplazo
                      tiene un precio de venta $
                      {warrantyPriceDiff.differenceUsd.toFixed(2)} USD mayor. La
                      empresa absorbe la diferencia.
                    </p>
                  )}
                </div>
              )}
            ...
            {warrantySettlementMode === "store_credit" && (
              <div className="space-y-3 rounded-md border border-dashed border-sky-200 bg-sky-50/50 p-4 dark:border-sky-900 dark:bg-sky-950/20">
                <p className="text-sm text-sky-800 dark:text-sky-200">
                  <strong>Saldo a favor del cliente:</strong> se creará una
                  venta pendiente de tipo "Garantía" que el cliente podrá cobrar
                  desde caja.
                </p>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    step="0.01"
                    placeholder={
                      warrantyPriceDiff.differenceArs
                  ...
                  <p className="text-xs text-muted-foreground">
                    ...
                    ARS
                    {fxRate &&
                      ` (USD ${Math.abs(warrantyPriceDiff.differenceUsd).toFixed(2)} × $${Number(fxRate).toLocaleString("es-AR")})`}
                  </p>
                </div>
              </div>
            )}
            ...
            {warrantySettlementMode === "customer_payment" && (
              <div className="space-y-3 rounded-md border border-dashed border-amber-200 bg-amber-50/50 p-4 dark:border-amber-900 dark:bg-amber-950/20">
                <p className="text-sm text-amber-800 dark:text-amber-200">
                  <strong>Cobro de diferencia:</strong> se creará una venta
                  pendiente de tipo "Garantía" que se cobrará desde caja.
                </p>
                <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={handleProcessWarranty}
              disabled={warrantyProcessing}
            >
      ...
      >
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={proceedToBucketSelection}>
              Siguiente
            </Button>
          </DialogFooter>
        </DialogContent>
              ...
              <div className="space-y-2 p-3 border rounded-lg bg-purple-50 dark:bg-purple-950/30">
                ...
                <label className="flex items-center gap-2 mt-2 cursor-pointer">
                  ...
                  <span className="text-sm">
                    Eliminar unidad ingresada por canje
                  </span>
                </label>
              </div>
            )}
      ...
      >
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Esta acción cancelará la venta y devolverá el stock al inventario.
              No se revierte dinero (la venta no fue cobrada).
            </p>
            {confirmCancelSale && (
              <p className="text-sm">
                ...
                <strong>
                  {formatPersonName(
                    confirmCancelSale.customer_name,
                    confirmCancelSale.customer_last_name,
                  )}
                </strong>
              </p>
            )}
