-- ===========================================================
-- Update create_canje_sale: IMEI for buy items + gifts
-- ===========================================================
...
BEGIN
  ...
  -- Calcular total_usd de los items a comprar (excluir regalos)
    ...
    IF coalesce(v_inventory_tracking_mode, 'quantity') = 'serial' THEN
      IF (item ->> 'is_gift')::boolean IS NOT TRUE THEN
        ...
        IF jsonb_typeof(item -> 'inventory_unit_ids') <> 'array' THEN
          RAISE EXCEPTION 'inventory_unit_ids debe ser un arreglo para la variante %', v_variant_id;
        END IF;
  ...
  IF v_tracking_mode = 'serial' THEN
    ...
    INSERT INTO public.inventory_units (
      ...
      received_at, notes, sale_id, updated_by
    ) VALUES (
