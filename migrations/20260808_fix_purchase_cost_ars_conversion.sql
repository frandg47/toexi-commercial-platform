...
begin
  ...
  -- Obtener cotizacion USD para conversion de costo cuando la compra es en ARS
  if p_currency = 'ARS' then
    select rate into v_usd_rate
    from public.fx_rates
    where is_active = true and source = 'blue'
    limit 1;
  else
    v_usd_rate := null;
  end if;
  ...
  loop
    ...
    if v_variant_id is null or v_quantity is null or v_unit_cost is null then
      raise exception 'Cada item debe tener variant_id, quantity y unit_cost';
    end if;

    if v_quantity <= 0 then
      raise exception 'La cantidad debe ser mayor a 0 para la variante %', v_variant_id;
    end if;
    ...
    set stock = coalesce(stock, 0) + v_quantity,
        cost_price_usd = case
          ...
          when p_currency = 'ARS' and v_usd_rate is not null and v_usd_rate > 0
            then v_unit_cost / v_usd_rate
          else cost_price_usd
        end,
        updated_at = now()
